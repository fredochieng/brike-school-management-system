...
$parent_info = $this->db->get_where('parent', array('student_id' => $student_id))->result_array();
foreach ($parent_info as $row1);
?>
            ...
            <center>
                <a href="#">
                    <img src="<?php echo $this->crud_model->get_image_url('student', $student_id); ?>"
                        class="img-circle" style="width: 60%;" />
                </a>
                <br>
                <h3>
                    <?php echo $row['name'] . " " . $row['middle_name'] . " " . $row['last_name']; ?>
                </h3>
                <br>
                <span>
                    <?php
                    ...
                    <a href="<?php echo site_url('admin/student_information/' . $enroll_info->row()->class_id); ?>">
                        <?php echo get_phrase('class') . ' - ' . $class_name . ' | ' . get_phrase('stream') . ' - ' . $section_name; ?>
                    </a>
                </span>
            </center>
        </div>
        <div class="col-md-9">
            ...
            <div class="tab-content">
                <div class="tab-pane active" id="tab1">
                    <?php
                    $basic_info_titles = ['name', 'gender', 'DOB', 'class', 'stream', 'admn_no', 'county', 'subcounty', 'address', 'dormitory'];
                    $basic_info_values = [
                        $row['name'] . " " . $row['middle_name'] . " " . $row['last_name'],
                        $row['sex'] == null ? '' : $row['sex'],
                        $row['birthday'],
                        $class_name, $section_name, $row['student_code'],
                        $row['county_id'] == null ? '' : $this->db->get_where('county', array('county_id' => $row['county_id']))->row()->name,
                        $row['phone'] == null ? '' : $row['phone'], $row['address'] == null ? '' : $row['address'],
                ...
                <div class="tab-pane" id="tab2">
                    ...
                    $parent_info_titles = ['name', 'id_number', 'email', 'phone', 'alt_phone'];
                    foreach ($parent_info as $info) {
                        $parent_info_values = [
                            $info['first_name'] . " " . $info['l_name'], $info['id_number'], $info['email'], $info['phone'] == null ? '' : $info['phone'],
                            $info['alt_phone'] == null ? '' : $info['alt_phone'],
                        ];
                    }
                    ?>
                    <table class="table table-bordered" style="margin-top: 20px;">
